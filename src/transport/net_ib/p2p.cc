...
// 前向声明全局变量（在 aimd_cc.cc 中定义，NCCL_CC_MAX_* 与 g_cc_pool 由 aimd_cc.h 提供）
extern struct ShadowRequest g_shadow_pool[NCCL_CC_MAX_QP][NCCL_CC_MAX_SHADOW_SLOTS];
extern struct CollectiveCC* g_cc_pool[NCCL_CC_MAX_COLLECTIVES];
extern pthread_mutex_t g_cc_pool_mutex;

// 前向声明（定义在 ncclIbTest 之前，供 ncclIbMultiSend 窗口满时 poll CQ 使用）
static ncclResult_t ncclIbLogCompletionWithError(struct ncclIbNetCommBase* commBase, struct ibv_wc* wc, int devIndex);
static ncclResult_t ncclIbCompletionEventProcess(struct ncclIbNetCommBase* commBase, struct ibv_wc* wc, int devIndex);
...
ncclResult_t ncclIbMultiSend(struct ncclIbSendComm* comm, int slot) {
  ...
  uint64_t original_lastWr_wr_id = lastWr->wr_id;  // 仅 lastWr 产生 CQE，供 AllocateShadowSlot；循环内每 QP 覆盖 lastWr->wr_id

  if (ncclIbIsAIMDEnabled()) {
    uint64_t collective_id = comm->base.fifoHead;
    
    // 按 comm 分配 CC，传 0 作为 estimated_chunks 得 max_chunks=1024
    ...
    if (cc && cc->enabled) {
      ...
      do {
        ...
        if (old_inflight >= cc->lib_window) {
          ...
          int wrDone = 0;
          int totalWrDone = 0;
          int totalCqeInRound = 0;  // 本轮窗口满期间 poll 到的 CQE 总数，用于诊断
          do {
            totalWrDone = 0;
            for (int i = 0; i < comm->base.vProps.ndevs; i++) {
              struct ncclIbNetCommDevBase* devBase = ncclIbGetNetCommDevBase(&comm->base, i);
              NCCLCHECK(wrap_ibv_poll_cq(devBase->cq, 8, wcs, &wrDone));
              if (wrDone == 0) continue;
              totalWrDone += wrDone;
              for (int w = 0; w < wrDone; w++) {
                struct ibv_wc* wc = wcs + w;
                if (wc->status != IBV_WC_SUCCESS) {
                  ncclIbLogCompletionWithError(&comm->base, wc, i);
                  return ncclRemoteError;
                }
                NCCLCHECK(ncclIbCompletionEventProcess(&comm->base, wc, i));
              }
              NCCLCHECK(ncclIbStatsCheckFatalCount(&ncclIbDevs[devBase->ibDevN].stats, __func__));
            }
            totalCqeInRound += totalWrDone;
          } while (totalWrDone > 0);

          // 排空后若 cqe_in_round==0：再做一次 yield + 单轮 poll，部分 CQE 可能已在回写路上，多消化几个再 return，不恢复多轮 for(wait)
          if (totalCqeInRound == 0) {
            sched_yield();
            totalWrDone = 0;
            for (int i = 0; i < comm->base.vProps.ndevs; i++) {
              struct ncclIbNetCommDevBase* devBase = ncclIbGetNetCommDevBase(&comm->base, i);
          ...
          if (now_us - last_print > 1000000) {
            ...
            if (comm->base.vProps.ndevs > 0) {
              ...
              if (_db) printf("[AIMD] blocked CQ: dev0 cq=%p\n", (void*)_db->cq);
            }
#endif
            fflush(stdout);
            last_print = now_us;
      ...
      for (int j = 0; j < NCCL_CC_MAX_COLLECTIVES; j++) {
        if (g_cc_pool[j] == cc) {
          cc_index = j;
          break;
        }
...
#endif
    // 构建WR链表（包括lastWr）
    struct ibv_send_wr* first_wr = comm->wrs;
    if (has_last_wr) {
      comm->wrs[nreqs-1].next = lastWr;
    } else {
      comm->wrs[nreqs-1].next = NULL;
    }
#ifdef AIMD_DEBUG
    if (cc && cc->enabled && !(lastWr->send_flags & IBV_SEND_SIGNALED)) {
      printf("[AIMD] [ANOMALY] lastWr->send_flags=0x%x missing IBV_SEND_SIGNALED\n", lastWr->send_flags);
      fflush(stdout);
    }
    if (cc && cc->enabled) {
      uint32_t magic = (uint32_t)((lastWr->wr_id >> 48) & 0xFFFF);
      int anomalous = (lastWr->wr_id == 0 || magic != NCCL_CC_WR_MAGIC);
      uint64_t now = ncclIbGetMicros();
      static uint64_t last_post_log = 0;
      if (anomalous || now - last_post_log > 500000) {
        printf("[AIMD] Posting: lastWr->wr_id=0x%llx magic=0x%x qp=%d has_last_wr=%d%s\n",
               (unsigned long long)lastWr->wr_id, magic, qp->qp->qp_num, has_last_wr,
               anomalous ? " [ANOMALY]" : "");
        fflush(stdout);
        if (!anomalous) last_post_log = now;
      }
    }
#endif
    ncclResult_t post_ret = wrap_ibv_post_send(qp->qp, first_wr, &bad_wr);
...
ncclResult_t ncclIbIsend(void* sendComm, void* data, size_t size, int tag, void* mhandle, void* phandle, void** request) {
  ...
  for (int r=0; r<nreqs; r++) {
    ...
    *request = reqs[r] = req;
    ...
    TIME_START(0);
    ncclResult_t ret = ncclIbMultiSend(comm, slot);
    if (ret == ncclInProgress) {
      // 窗口满未 post：*request=NULL 让上层重试，不 advance 以保留 slot；返回 ncclInProgress 使上层知悉需再次 progress，test 其它 in-flight 的 req 后 CQE 被 poll、inflight 下降
      *request = NULL;
      TIME_STOP(0);
      return ncclInProgress;
    }
...
static inline ncclResult_t ncclIbCompletionEventProcess(struct ncclIbNetCommBase* commBase, struct ibv_wc* wc, int devIndex) {
  // 🔴 AIMD拥塞控制：先处理受控WR
  if (ncclIbIsAIMDEnabled()) {
    ncclResult_t cc_ret = ncclIbOnCompletionWithCC(commBase, wc, devIndex);
    if (cc_ret != ncclSuccess) {
      return cc_ret;
    }
    // 如果已经处理了受控WR，需要检查是否还需要继续NCCL原生逻辑
    // 注意：ncclIbOnCompletionWithCC会还原wr_id，所以可以继续处理
  }
  ...
  *done = 0;
  ...
  do {
    ...
    for (int i = 0; i < NCCL_IB_MAX_DEVS_PER_NIC; i++) {
      ...
      totalWrDone += wrDone;
#ifdef AIMD_DEBUG
      { static uint64_t last_t=0; uint64_t t=ncclIbGetMicros(); if (t-last_t>500000) { printf("[AIMD] Test got CQE: r->base=%p\n", (void*)r->base); last_t=t; } }
#endif
      for (int w=0; w<wrDone; w++) {
        struct ibv_wc *wc = wcs+w;
        if (wc->status != IBV_WC_SUCCESS) {
