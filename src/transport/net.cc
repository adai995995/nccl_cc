...
static ncclResult_t sendProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    int anyIsendInProgress = 0;  // AIMD: 若本轮有 Isend 因窗口满返回 ncclInProgress，之后多做一轮全 sub Test
    for (int s=0; s<args->nsubs; s++) {
      struct ncclProxySubArgs* sub = args->subs+s;
      ...
      // AIMD: 先 Test 再 Isend，便于在窗口满时先消化 CQE、降低 inflight，再尝试发送，缓解 MultiSend blocked 后 cqe_in_round=0 的卡死。
      ...
      if (sub->transmitted < sub->posted && sub->transmitted < sub->done + NCCL_STEPS) {
        ...
        if (connFifo[buffSlot].size != -1 && (*recvTail > tail || p == NCCL_PROTO_LL)) {
          ...
          if (ready) {
            ...
            if (sub->requests[buffSlot] == NULL) anyIsendInProgress = 1;  // AIMD 窗口满返回 ncclInProgress
      ...
      // AIMD: Isend 之后再 Test 一次，Isend 返回 ncclInProgress(窗口满) 时本 round 仍能多消化一次 CQE，缓解 blocked
      if (sub->done < sub->transmitted) {
        int done;
        int size;
        int buffSlot = (sub->base+sub->done)%NCCL_STEPS;
        NCCLCHECK(proxyState->ncclNet->test(sub->requests[buffSlot], &done, &size));
        if (done) {
          int stepJustDone = sub->done;
          connFifo[buffSlot].size = -1;
          std::atomic_thread_fence(std::memory_order_seq_cst);
          TRACE(NCCL_NET, "sendProxy [%ld/%d/%d] request %p done (post-Isend)", sub->done, buffSlot, sub->nsteps, sub->requests[buffSlot]);
          sub->done += args->sliceSteps;
          ncclProfilerStopProxyStepEvent(s, args, stepJustDone);
          if (resources->shared == 0) {
            volatile uint64_t* sendHead = resources->gdcSync ? resources->gdcSync : &resources->sendMem->head;
            *sendHead = sub->base + sub->done;
    ...
    // AIMD: 若本轮有 Isend 返回 ncclInProgress（窗口满），多做一轮全 sub Test，多消化 CQE 以尽快降低 inflight
    if (anyIsendInProgress) {
      for (int s=0; s<args->nsubs; s++) {
        struct ncclProxySubArgs* sub = args->subs+s;
        if (sub->done >= sub->transmitted || sub->done == sub->nsteps) continue;
        struct sendNetResources* resources = (struct sendNetResources*) (sub->connection->transportResources);
        volatile struct ncclConnFifo* connFifo = (volatile struct ncclConnFifo*)resources->recvMem->connFifo;
        int buffSlot = (sub->base+sub->done)%NCCL_STEPS;
